    ...
    public List<IJobBuilder> GetJobs()
    {
        ...
        foreach (var package in settings.Wrench.PackagesToRelease)
        {
            ...
            var job = JobBuilder.Create(GetJobName(settings.EditorVersion, platform.System))
                ...
                .WithCommands($"upm-ci package test -u trunk --type package-tests --enable-code-coverage --code-coverage-options \"generateAdditionalMetrics;generateHtmlReport;assemblyFilters:+{packageAssemblyName}*,-*Tests*,-*Examples*;pathFilters:-**Tests**,-**External**;\" --extra-utr-arg=--coverage-results-path={sourceDir}/upm-ci~/test-results/CoverageResults")
                .WithUpmCiArtifacts()
