...
namespace Parabox.UVee
{
	class UVeeWindow : EditorWindow
	{
		...
		internal class MeshSelection
		{
			...
			{
				get { return meshFilter != null ? meshFilter.gameObject : (skinnedMeshRenderer != null ? skinnedMeshRenderer.gameObject : null); }
			}

			public Renderer renderer
			{
				get { return meshFilter != null ? (Renderer)meshFilter.GetComponent<MeshRenderer>() : (Renderer)skinnedMeshRenderer; }
			}

			public Mesh sharedMesh
			{
				get { return meshFilter != null ? meshFilter.sharedMesh : (skinnedMeshRenderer == null ? null : skinnedMeshRenderer.sharedMesh); }
			...
			{
				get { return (Object)meshFilter ?? (Object)skinnedMeshRenderer; }
			}
		}
		...
		// selection caches
		int[][] distinct_triangle_selection = new int[0][];
		///< Guarantees that only one index per vertex is present
		...
		public bool UndoRedoPerformedEvent
		{
			get { return Event.current.type == EventType.ValidateCommand; }
		...
		void GenerateUV2(MeshSelection[] selection)
		{
			for (int i = 0; i < selection.Length; i++)
			{
		...
		Vector2[] UVArrayWithTriangles(MeshSelection mf, int[] tris)
		{
	...
	static class TransformExtensions
	{
		...
		internal static Vector3[] VerticesInWorldSpace(UVeeWindow.MeshSelection mf)
		{
