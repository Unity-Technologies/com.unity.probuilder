...
static const float ORTHO_CAM_OFFSET = .0001;
...
{
    float4 ret = float4(UnityObjectToViewPos(pos), 1.0);
    ...
    ret.xyz *= lerp(0.99, 1, unity_OrthoParams.w);
    //Moving edges closer
	//.99 is not sufficient for Orthographic Camera
    ret.z *= lerp(1.0, 0.95, unity_OrthoParams.w);
...
inline float4 GetPickerColor(float4 pos, float2 texcoord1)
{
    ...
    clip.z -= 0.0001 * (1.0 - UNITY_MATRIX_P[3][3]);
