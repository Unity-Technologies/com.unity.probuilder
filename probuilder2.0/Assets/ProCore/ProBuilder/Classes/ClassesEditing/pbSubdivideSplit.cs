...
#region Internal Implementation
	...
	private static bool SplitFace_Internal(SplitSelection splitSelection,
		...
		out List<List<Vector4>> splitUVs,
		out int[][] splitSharedIndices) 
	{
 	...
 	// 	Vector2 splitPointB_uv = splitSelection.bIsVertex ? pb.uv[splitSelection.indexB[0]] : (pb.uv[splitSelection.indexB[0]] + pb.uv[splitSelection.indexB[1]]) / 2f;
		...
		// List<Vector4> u_polyA = new List<Vector4>();
		// List<Vector4> u_polyB = new List<Vector4>();

		// List<Vector4> u3_polyA = uv3 == null ? null : new List<Vector4>();
		// List<Vector4> u3_polyB = uv3 == null ? null : new List<Vector4>();

		// List<Vector4> u4_polyA = uv4 == null ? null : new List<Vector4>();
		// List<Vector4> u4_polyB = uv4 == null ? null : new List<Vector4>();
		...
		// 		if(uv3 != null) u3_polyA.Add( uv3[i] );
		// 		if(uv3 != null) u3_polyB.Add( uv3[i] );
				
		// 		if(uv4 != null) u4_polyA.Add( uv4[i] );
		// 		if(uv4 != null) u4_polyB.Add( uv4[i] );
		...
		// 	u_polyA.Add( (splitSelection.aIsVertex ? pb.uv0[splitSelection.indexA[0]] : (pb.uv0[splitSelection.indexA[0]] + pb.uv0[splitSelection.indexA[1]]) * .5f) );
		// 	if(uv3 != null) u3_polyA.Add( (splitSelection.aIsVertex ? pb.uv3[splitSelection.indexA[0]] : (pb.uv3[splitSelection.indexA[0]] + pb.uv3[splitSelection.indexA[1]]) * .5f) );
		// 	if(uv4 != null) u4_polyA.Add( (splitSelection.aIsVertex ? pb.uv4[splitSelection.indexA[0]] : (pb.uv4[splitSelection.indexA[0]] + pb.uv4[splitSelection.indexA[1]]) * .5f) );
		// 	i_polyA.Add(-1);
		// 	c_polyA.Add(splitSelection.colorA);
			
		// 	v_polyB.Add( splitPointA_3d );
		// 	v_polyB_2d.Add( splitPointA_2d );
		// 	u_polyB.Add( (splitSelection.aIsVertex ? pb.uv0[splitSelection.indexA[0]] : (pb.uv0[splitSelection.indexA[0]] + pb.uv0[splitSelection.indexA[1]]) * .5f) );
		// 	if(uv3 != null) u3_polyB.Add( (splitSelection.aIsVertex ? pb.uv3[splitSelection.indexA[0]] : (pb.uv3[splitSelection.indexA[0]] + pb.uv3[splitSelection.indexA[1]]) * .5f) );
		// 	if(uv4 != null) u4_polyB.Add( (splitSelection.aIsVertex ? pb.uv4[splitSelection.indexA[0]] : (pb.uv4[splitSelection.indexA[0]] + pb.uv4[splitSelection.indexA[1]]) * .5f) );
		...
		out int[][] splitSharedIndices)
	{
		...
		// List<Vector4> uv3 = pb.uv3;
		// List<Vector4> uv4 = pb.uv4;

		// List<Vector4> edgeCentersUV = new List<Vector4>();
		// List<Vector4> edgeCentersUV3 = new List<Vector4>();
		// List<Vector4> edgeCentersUV4 = new List<Vector4>();
		...
		// 		edgeCentersUV.Add( (uvs[edge.x] + uvs[edge.y]) * .5f );
		// 		if(uv3 != null) edgeCentersUV3.Add( (uv3[edge.x] + uv3[edge.y]) * .5f );
		// 		if(uv4 != null) edgeCentersUV4.Add( (uv4[edge.x] + uv4[edge.y]) * .5f );
		...
		// Vector4[] faceUVs = pb.uv0.ValuesWithIndices(face.distinctIndices);
		// Color[] colors = pbUtil.ValuesWithIndices(pb.colors, face.distinctIndices);
		...
		// Vector4 cenUV = pb_Bounds2D.Center(faceUVs.Cast<Vector2>().ToArray());
		// Vector4 cenUV3 = uv3 == null ? Vector4.zero : pb_Bounds2D.Center(faceUVs.Cast<Vector2>().ToArray());
		// Vector4 cenUV4 = uv4 == null ? Vector4.zero : pb_Bounds2D.Center(faceUVs.Cast<Vector2>().ToArray());
		...
		// List<Vector4>[] quadrantsUV = new List<Vector4>[edgeCenters2d.Length];
		// List<Vector4>[] quadrantsUV3 = new List<Vector4>[edgeCenters2d.Length];
		// List<Vector4>[] quadrantsUV4 = new List<Vector4>[edgeCenters2d.Length];
