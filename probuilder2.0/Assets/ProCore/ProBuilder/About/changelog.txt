...
- Fix null reference when undoing a delete.
- Fix torus and icosphere not remembering last used settings in a session.
- Use semantic versioning format for build type and number.

# ProBuilder 3.0.0-b.3

## Changes from 3.0.0b2

- Fix scene repainting endlessly when drag selecting elements.
- Improve performance of pb_Editor selection caching, fixing lag when selecting high vertex count objects.
- Fix scene toolbar sometimes not matching free/pro skin.
- Fix washed colors in gui textures with linear rendering.
- Default new objects with Static flag checked, and add preference to set a custom default.
- Fix About window failing to find correct build type.
- Fix Standard Vertex Color shader errors in 2018.1 and up.
- Match element drag rect image to Unity's drag rect.
- Remove unused preferences from the ProBuilder/Preferences window.
- Fix Poly Shape editor repainting the sceneview on every tick.
- Fix "Select Hidden" preference not matching editor behavior at launch.

# ProBuilder 3.0.0b2

## Changes from 3.0.0b1

- Fix "Select Hidden" preference not matching the state shown in toolbar on launch.

# ProBuilder 3.0.0b1

## Changes from 3.0.0b0

- Remove deprecated `pb_Entity` repair actions.
- Fix edge drag selection regression.
- Fix UPM warnings on import due to icons post-processor running.
- Fix UPM warnings due to missing meta files.

# ProBuilder 3.0.0b0

## Features

- Custom color palettes now supported.
- Add an editor window to display licenses for third party software used in ProBuilder.
...
- Support partial and complete rect selection for edge elements.
...
- Fix potential null ref if a menu action is called without a pb_Editor instance.
...
- Add button to view third party licenses in About window.
- Debug symbols now distributed libraries.
- Enable custom color palettes in the Vertex Color Palette window.

## Bug Fixes

- Fix entity vis toggles using pro tint in basic skin.
- Fix standard shader errors in 2017.3.
- Corrected tool-tips for "Clear Smoothing" and "Break Smoothing" buttons.

## Changes

- Move Dimensions Overlay into ProBuilder menu.
- Move pb_Object/Entity to ProBuilder.Core namespace.
- Move `ProBuilder2.Common` and` ProBuilder2.EditorCommon` namespaces to `ProBuilder.Core` and `ProBuilder.EditorCore`.

## Known Issues

- Standard Vertex Color shadder throws warning about semicolon.
- "Library already loaded" logs when importing Package Manager over Asset Store install.
