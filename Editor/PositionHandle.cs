...
namespace SlideyDebuggey
{
    public static class PositionHandle
    {
            ...
            public static PositionHandleIds @default
            {
                get
                {
                    return new PositionHandleIds(
                        GUIUtility.GetControlID("s_xAxisMoveHandleHash".GetHashCode(), FocusType.Passive),
                        GUIUtility.GetControlID("s_yAxisMoveHandleHash".GetHashCode(), FocusType.Passive),
                        GUIUtility.GetControlID("s_zAxisMoveHandleHash".GetHashCode(), FocusType.Passive),
                        GUIUtility.GetControlID("s_xyAxisMoveHandleHash".GetHashCode(), FocusType.Passive),
                        GUIUtility.GetControlID("s_xzAxisMoveHandleHash".GetHashCode(), FocusType.Passive),
                        GUIUtility.GetControlID("s_yzAxisMoveHandleHash".GetHashCode(), FocusType.Passive),
                        GUIUtility.GetControlID("s_FreeMoveHandleHash".GetHashCode(), FocusType.Passive)
                    );
                }
            }
        ...
        static Vector3 DoPositionHandle_Internal(PositionHandleIds ids, Vector3 position, Quaternion rotation, PositionHandleParam param)
        {
            ...
            var viewVectorDrawSpace = Handles.GetCameraViewFrom(handlePosition, invDrawToWorldMatrix);

            var size = HandleUtility.GetHandleSize(position);

            // Calculate per axis camera lerp
            for (var i = 0; i < 3; ++i)
                s_DoPositionHandle_Internal_CameraViewLerp[i] = ids[i] == GUIUtility.hotControl ? 0 : Handles.GetCameraViewLerpForWorldAxis(viewVectorDrawSpace, Handles.GetAxisVector(i));
            ...
            //     if (cameraLerp <= Handles.kCameraViewThreshold)
            //     {
            ...
            for (var i = 0; i < 3; ++i)
            {
                ...
                var axisColor = Handles.GetColorByAxis(i);
                Handles.color = isDisabled ? Color.Lerp(axisColor, Handles.staticColor, Handles.staticBlend) : axisColor;
                ...
                if (cameraLerp <= Handles.kCameraViewThreshold)
                {
                    ...
                    var axisVector = Handles.GetAxisVector(i);
                    ...
                    s_DoPositionHandle_ArrowCapConeOffset = isHot
                        ...
                        : Vector3.zero;
                    // Do(int id, Vector3 position, Vector3 offset, Vector3 handleDirection, Vector3 slideDirection, float size, Handles.CapFunction capFunction, float snap)

                    position = Slider1D.Do(ids[i], position, offset, dir, dir, size * param.axisSize[i], DoPositionHandle_ArrowCap, GridSnapping.active ? 0f : EditorSnapSettings.move[i]);
                }
            }
            ...
            //     position = Handles.FreeMoveHandle(ids.xyz, position, rotation, size * kFreeMoveHandleSizeFactor, GridSnapping.active ? Vector3.zero : EditorSnapSettings.move, RectangleHandleCap);
            // }
        ...
        {
            Handles.ArrowHandleCap(controlId, position, rotation, size, eventType, s_DoPositionHandle_ArrowCapConeOffset);
        }
