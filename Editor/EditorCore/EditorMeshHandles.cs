...
namespace UnityEditor.ProBuilder
{
    partial class EditorMeshHandles : ScriptableSingleton<EditorMeshHandles>, IHasPreferences
    {
        ...
        Dictionary<ProBuilderMesh, MeshHandle> m_TemporaryHandles;
        ...
        void OnEnable()
        {
            ...
            m_TemporaryHandles = new Dictionary<ProBuilderMesh, MeshHandle>();
        ...
        public static void DrawSceneSelection(SceneSelection selection)
        {
            ...
            using(var drawingScope = new LineDrawingScope(s_PreselectionColor, mesh.transform.localToWorldMatrix, -1f,
                CompareFunction.Always))
            {
        ...
        void DrawSceneHandlesInternal(SelectMode mode)
        {
            ...
            switch(mode)
            {
                ...
                case SelectMode.TextureEdge:
                {
                    ...
                    Render(m_WireHandles, m_ForceEdgeLinesGL ? m_GlWireMaterial : m_EdgeMaterial, s_EdgeUnselectedColor,
                        CompareFunction.LessEqual, false);
                    Render(m_SelectedEdgeHandles, m_ForceEdgeLinesGL ? m_GlWireMaterial : m_EdgeMaterial,
                        s_EdgeSelectedColor, s_DepthTestHandles ? CompareFunction.LessEqual : CompareFunction.Always,
                        true);
                    break;
                }
        ...
        void ClearHandlesInternal()
        {
            ...
            ClearHandlesInternal(m_TemporaryHandles);
        }
        ...
        void RebuildMeshHandle(ProBuilderMesh mesh, Dictionary<ProBuilderMesh, MeshHandle> list,
            Action<ProBuilderMesh, Mesh> ctor)
        {
            var handle = GetMeshHandle(mesh, list);
            ctor(mesh, handle.mesh);
        }

        void RebuildMeshHandleFromFaces(ProBuilderMesh mesh, IList<Face> faces,
        ...
        public static void HighlightFaces(ProBuilderMesh mesh, IList<Face> faces)
        {
            instance.HighlightFacesInternal(mesh, faces, s_FaceSelectedColor);
        }

        public static void HighlightFaces(ProBuilderMesh mesh, IList<Face> faces, Color highlightColor)
        {
            instance.HighlightFacesInternal(mesh, faces, highlightColor);
        }

        private void HighlightFacesInternal(ProBuilderMesh mesh, IList<Face> faces, Color highlightColor)
        {
            // Dictionary<ProBuilderMesh, MeshHandle> wireHandles = new Dictionary<ProBuilderMesh, MeshHandle>();
            // if(m_ForceWireframeLinesGL || BuiltinMaterials.geometryShadersSupported)
            ...
            //Render(wireHandles, m_ForceWireframeLinesGL ? m_GlWireMaterial : m_WireMaterial, s_WireframeColor, CompareFunction.LessEqual, false);

            RebuildMeshHandleFromFaces(mesh, faces, m_TemporaryHandles, MeshHandles.CreateFaceMeshFromFaces);
        ...
        {
            instance.HighlightEdgesInternal(mesh, edges, highlightColor);
        }

        private void HighlightEdgesInternal(ProBuilderMesh mesh, IList<Edge> edges, Color highlightColor)
        {
            var handle = GetMeshHandle(mesh, m_TemporaryHandles);
        ...
        {
            instance.HighlightVerticesInternal(mesh, vertexIndexes, highlightColor);
        }

        private void HighlightVerticesInternal(ProBuilderMesh mesh, IList<int> vertexIndexes, Color highlightColor)
        {
            var handle = GetMeshHandle(mesh, m_TemporaryHandles);
            MeshHandles.CreateVertexMesh(mesh, handle.mesh, vertexIndexes);

            Render(m_TemporaryHandles, m_VertMaterial, highlightColor, s_DepthTestHandles);
        }
    }
}
