...
namespace UnityEditor.ProBuilder
{
	sealed class VertexColorPainter : EditorWindow
	{
		...
		{
			EditorWindow.GetWindow<VertexColorPainter>(true, "ProBuilder Vertex Painter", true).Show();
		}
		...
		public ProBuilderEditor editor
		{
			...
			else
			{
				...
				for (int i = 0; i < (int) Mathf.Min(textures.Length, 4); i++)
				{
					...
					if (i < textures.Length && i == GetIndex(color))
					{
						...
						UI.EditorGUIUtility.DrawSolidColor(r, Color.green);
					}
				}
				GUILayout.EndHorizontal();
		...
		void ColorGUI()
		{
			...
			UI.EditorGUIUtility.DrawSolidColor(r, color);
		...
		void OnSceneGUI(SceneView scnview)
		{
			 ...
			 * Hit test scene
			 */
			if (!lockhandleToCenter && !EditorHandleUtility.SceneViewInUse(currentEvent))
			{
				if (pb != null)
				{
